...
import {currentPlayer, freePositionY, winingPositions} from "../functions/game";
...
export const dropTokenAction: GameAction<"dropToken"> = ({grid, players}, {playerId, x: eventX}) => {
...
export const saveWiningPositionsAction: GameAction<"dropToken"> = (context, event) => ({
    winingPositions: winingPositions(context.grid, currentPlayer(context).color!, event.x, context.rowLength)
})
